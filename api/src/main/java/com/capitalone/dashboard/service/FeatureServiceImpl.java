...
public class FeatureServiceImpl implements FeatureService {
    ...
    public DataResponse<List<Feature>> getRelevantStories(ObjectId componentId, String teamId) {
        ...
        // component
        List<Feature> relevantStories = featureRepository.queryByOrderBySStatusDescUpdate(teamId,
    ...
    public DataResponse<List<Feature>> getFeatureEstimates(ObjectId componentId, String teamId) {
        ...
        List<Feature> relevantFeatureEstimates = featureRepository
                .getInProgressFeaturesEstimatesByTeamIdUpdate(teamId, getCurrentISODateTime());
    ...
    private DataResponse<List<Feature>> getEstimate(ObjectId componentId, String teamId, Status status) {
    	LOGGER.info("***IN Method getEstimate***  with teamid  "+teamId);
        Component component = componentRepository.findOne(componentId);
        if ((component == null) || CollectionUtils.isEmpty(component.getCollectorItems()) ||
                CollectionUtils.isEmpty(component.getCollectorItems().get(CollectorType.ScopeOwner)) ||
                (component.getCollectorItems().get(CollectorType.ScopeOwner).get(0) == null)) {
        ...
        builder.and(team.collectorItemId.eq(item.getId()));
        LOGGER.info("*************builder and query executed*********************with item.getId() as"+item.getId());
        // Get teamId first from available collector item, based on component
        List<Feature> storyEstimates;
        switch (status) {
            case TOTAL:
            	
                storyEstimates = featureRepository.getSprintBacklogTotalUpdate(teamId,
                        getCurrentISODateTime());
                LOGGER.info("*************TOTAL ESTIMATE******************************"+storyEstimates.size());
                break;

            case DONE:
            	LOGGER.info("***Done case in getEstimate***  with teamid  "+teamId+"calling getSprintBacklogDoneUpdate");
                storyEstimates = featureRepository.getSprintBacklogDoneUpdate(teamId.trim(),
                        getCurrentISODateTime());
                LOGGER.info("*************DONE ESTIMATE *****************************"+storyEstimates.size());

                break;

            case InProgress:
            	
                storyEstimates = featureRepository.getSprintBacklogInProgressUpdate(teamId,
                        getCurrentISODateTime());
                LOGGER.info("*************In PROGRESS*****************************"+storyEstimates.size());
                break;
        ...
        LOGGER.info("**sum of estimated time is***"+ lineTotalEstimate);
        f.setsEstimate(Integer.toString(lineTotalEstimate));
    ...
    public DataResponse<List<Feature>> getTotalEstimate(ObjectId componentId, String teamId) {
    	LOGGER.info("***inside get total estimate: values passed are componenet id "+componentId+"team id is "+ teamId);
    ...
    public DataResponse<List<Feature>> getInProgressEstimate(ObjectId componentId, String teamId) {
    	LOGGER.info("***inside get in progress estimate: values passed are componenet id "+componentId+"team id is "+ teamId);
    ...
    public DataResponse<List<Feature>> getDoneEstimate(ObjectId componentId, String teamId) {
    	LOGGER.info("***inside get done estimate: values passed are componenet id "+componentId+"team id is "+ teamId);
    ...
    public DataResponse<List<Feature>> getCurrentSprintDetail(ObjectId componentId, String teamId) {
    	...
    	LOGGER.info("entered getCurrentSprintDetail with component id as "+componentId+"team id as "+teamId);
        Component component = componentRepository.findOne(componentId);
        if ((component == null) || CollectionUtils.isEmpty(component.getCollectorItems()) ||
                CollectionUtils.isEmpty(component.getCollectorItems().get(CollectorType.ScopeOwner)) ||
                (component.getCollectorItems().get(CollectorType.ScopeOwner).get(0) == null)) {
        	LOGGER.info("*************got empty data response from getcurrentSprintDetails**********  leaving method*********");
            return getEmptyDataResponse();
        }
        ...
        // Get teamId first from available collector item, based on component
        List<Feature> sprintResponse = featureRepository.getCurrentSprintDetailUpdate(teamId,
                getCurrentISODateTime());
        ...
        LOGGER.info("got current sprint details with no of sprints "+sprintDetail.size());
        Collector collector = collectorRepository.findOne(item.getCollectorId());
